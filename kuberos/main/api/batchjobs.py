...
class BatchJobNameListView(generics.ListAPIView):
    """
    List API view for auto completion in CLI
    """
    def list(self, request):
        batchjobs = BatchJobDeployment.objects.filter(created_by=request.user, is_active=True)
        serializer = BatchJobNameListSerializer(batchjobs, many=True)
