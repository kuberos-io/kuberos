...
class BatchJobDeployment(UserRelatedBaseModel):
    ...
    def get_resource_usages(self):
        """
        Get resource usages of all nodes in the cluster.
        """
        resource_usages = []
        try:
            for cluster in self.exec_clusters.all():
                cluster_usages = cluster.get_resource_usages(
                    start_time=self.started_at,
                    end_time=self.completed_at
                )
                for item in cluster_usages:
                    resource_usages.append({
                        'timestamp': (item['timestamp'] - self.started_at).seconds,
                        'usage': item['usage'],
                    })

        except:
            logger.warning("Error by getting resource usage. Start: %s, End: %s", self.started_at, self.completed_at)
    ...
    @property
    def edge_resource_group(self) -> bool:
        """
        Check whether using resource group for batchjob execution.
        Default: False
        """
        ...
        if metadata:
            resource_group = metadata.get('edgeResourceGroup', 'public')
