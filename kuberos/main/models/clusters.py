...
class Cluster(BaseTagModel):
    ...
    def get_cluster_state_for_batchjobs(self, 
                                        use_robot=True,
                                        edge_resource_group=['public']) -> dict:
        """
        ...
        for node in self.cluster_node_set.all():
            if not node.is_available():
                continue
            if not node.resource_group in edge_resource_group:
                continue
    ...
    def get_resource_usages(self,
                            start_time,
                            end_time) -> dict:
        usage_records = self.cluster_resource_usage_set.filter(
    ...
    def get_cluster_resource_usages(self) -> None:
        usages = []
    ...
    def get_reserved_ip_block(self):
        pass
...
class ClusterResourceUsage(BaseModel):
    
    cluster = models.ForeignKey(
        Cluster,
        related_name='cluster_resource_usage_set',
        on_delete=models.CASCADE
    )
...
class ClusterIPReservation(BaseModel):
    
    cluster = models.ForeignKey(
        Cluster,
        related_name='cluster_ip_reserve_set',
        on_delete=models.CASCADE
    )
    ...
    ip_allocated = models.JSONField(
        null=True,
        blank=True,
        default=list,
    )
    
    ip_available = models.JSONField(
        null=True,
        blank=True,
        default=list
    )
    ...
    def get_ips(self,
                ...
                in_block=True) -> list:
        """
        Get a block of IP address
        """
        ips = []
        
        ip_base, ip_range = self.ip_block.split('/')
        ip_range = 2**(32-int(ip_range))
        available_ips = [self.increment_ip(ip_base, i) for i in range(int(ip_range))]
        # print(available_ips)
        
        if not self.ip_allocated:
            self.ip_allocated = []
        
        for i in range(ip_range):
            ip = available_ips.pop(0)
            if ip not in self.ip_allocated:
                ips.append(ip)
                self.ip_allocated.append(ip)
                if len(ips) == num_ip - 1:
                    break
                continue
        
        print(f"Number of requested ips: {num_ip}, Number of IP allocated: {len(ips)}")
        self.save()
        
        return ips
        
    def increment_ip(self, ip, increment):
        a, b, c, d = map(int, ip.split("."))
        total = a*256**3 + b*256**2 + c*256 + d + increment
        return f"{total//256**3}.{(total//256**2)%256}.{(total//256)%256}.{total%256}"
    
    
    def release_ips(self, ips: list) -> None:
        for ip in ips:
            try:
                self.ip_allocated.remove(ip)
                logger.info("IP %s is released", ip)
            except ValueError:
                logger.warning(f"IP {ip} is not allocated")
        self.save()
